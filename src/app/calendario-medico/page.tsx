...
import DetalleGuardiaMedicoModal from '@/components/DetalleGuardiaMedicoModal' // <--- IMPORTAMOS EL MODAL
...
export default function CalendarioMedico() {
  ...
  const [myApplications, setMyApplications] = useState<string[]>([]) // Lista de IDs a las que apliqué
  ...
  async function fetchData() {
    ...
    // 1. Traemos guardias abiertas y las mías (verdes y grises)
    ...
    // 2. Traemos mis postulaciones para saber cuáles son Naranjas
      ...
      <div className="w-full max-w-[1400px] h-full flex flex-col">
        ...
        <div className="flex-1 flex flex-col bg-white rounded-xl shadow-xl border-2 border-slate-900 overflow-hidden min-h-0">
          <div className="grid grid-cols-7 bg-slate-900 border-b border-slate-900 shrink-0">
            {weekDays.map(day => (
              <div key={day} className="py-3 text-center text-xs font-bold text-white uppercase tracking-wider">{day}</div>
            ))}
          </div>
                ...
                return (
                  <div key={day.toString()} className={`p-2 transition-colors ${isCurrentMonth ? 'bg-white' : 'bg-slate-50'} flex flex-col`}>
                    <div className={`flex justify-end mb-2 shrink-0`}>
                      <span className={`text-xs font-bold h-7 w-7 flex items-center justify-center rounded-full ${isToday ? 'bg-slate-900 text-white shadow-md' : 'text-slate-500'}`}>{format(day, 'd')}</span>
                    </div>
                      ...
                      {dayShifts.map(shift => {
                        ...
                        return (
                          <div 
                            ...
                            className={`text-xs p-2 rounded-lg transition-all cursor-pointer group ${cardStyle}`}
                          >
                            <div className="font-semibold truncate">{shift.clinic?.full_name || 'Clínica'}</div>
                            <div className={`flex justify-between items-center mt-1`}>
                              <span>{format(shiftDate, 'HH:mm')}</span>
                              <span className="font-bold">${shift.price / 1000}k</span>
                            </div>
                          </div>
                        )
                      })}
