  ...
  return (
    <main className="h-[calc(100vh-73px)] flex justify-center items-start bg-slate-50 p-4 md:p-6 overflow-hidden">
      <div className="w-full max-w-[1400px] h-full flex flex-col">
        <div className="flex flex-col md:flex-row justify-between items-center mb-4 bg-slate-900 p-4 rounded-xl shadow-lg border border-slate-800 shrink-0 gap-4 md:gap-0">
        ...
        <div className="flex gap-3 md:gap-4 mb-2 text-[10px] md:text-xs justify-center md:justify-end px-2 shrink-0 font-medium text-slate-500 flex-wrap">
        ...
        {/* CONTENEDOR TÁCTIL (Aquí ocurre la magia del scroll horizontal) */}
        <div className="flex-1 flex flex-col bg-white rounded-xl shadow-xl border-2 border-slate-900 overflow-hidden min-h-0">
          <div className="flex-1 overflow-auto custom-scrollbar">
            <div className="min-w-[800px] h-full flex flex-col">
              ...
              <div className="grid grid-cols-7 bg-slate-900 border-b border-slate-900 shrink-0 sticky top-0 z-10">
                {weekDays.map(day => (<div key={day} className="py-3 text-center text-xs font-bold text-white uppercase tracking-wider">{day}</div>))}
              </div>
              
              <div className="grid grid-cols-7 auto-rows-[minmax(120px,auto)] md:auto-rows-[minmax(140px,auto)] gap-[1px] bg-slate-200 flex-1">
