...
export default function ExplorarGuardiaModal({
  ...
  isVerified: isVerifiedProp, doctorSpecialty, doctorSpecialtyVerified,
}: {
  shift: any
  onClose: () => void
  onRefresh?: () => void
  ...
  doctorSpecialtyVerified?: string[]
}) {
  const clinicId = shift?.clinic_id || shift?.clinic?.id
  const c = shift?.clinic || {}

  const [loading, setLoading] = useState(false)
  const [isVerified, setIsVerified] = useState(isVerifiedProp ?? false)
  const [checkingAuth, setCheckingAuth] = useState(isVerifiedProp === undefined)
  const [hasApplied, setHasApplied] = useState(hasAppliedProp ?? false)
  const [clinicProfile, setClinicProfile] = useState({
    avatar_url: c.avatar_url ?? null as string | null,
    cover_url: c.cover_url ?? null as string | null,
    num_doctors: c.num_doctors ?? null as number | null,
    num_nurses: c.num_nurses ?? null as number | null,
    resources: parseArr(c.resources),
    rating: c.clinic_rating ?? c.rating ?? null as number | null,
    reviews_count: c.clinic_reviews_count ?? c.reviews_count ?? null as number | null,
  })
  ...
  useEffect(() => {
    if (isVerifiedProp !== undefined) return
    supabase.auth.getUser().then(({ data: { user } }) => {
      if (user) {
        supabase.from('accounts').select('verified_at').eq('id', user.id).single()
          .then(({ data }) => setIsVerified(!!data?.verified_at))
      }
  ...
  useEffect(() => {
    ...
    Promise.all([
      supabase.from('accounts_public')
        .select('avatar_url, cover_url, num_doctors, num_nurses, resources, clinic_rating, clinic_reviews_count')
        .eq('id', clinicId)
        .single(),
    ]).then(([profileRes]) => {
      if (profileRes.data) {
        ...
        setClinicProfile({
          ...
          resources: parseArr(r.resources),
          rating: r.clinic_rating,
          reviews_count: r.clinic_reviews_count,
        })
      }
    })
    if (hasAppliedProp === undefined) {
      supabase.auth.getUser().then(({ data: { user } }) => {
        if (user) {
          supabase.from('shift_applications').select('id').eq('shift_id', shift.id).eq('doctor_id', user.id).maybeSingle()
            .then(({ data }) => setHasApplied(!!data))
        }
      })
    }
  }, [shift?.id, clinicId, hasAppliedProp])
  ...
  const location = shift?.clinic?.clinic_location || shift?.clinic?.location_maps || shift?.clinic?.address || '—'
  ...
  const specArr = doctorSpecialty ?? []
  const specVerified = doctorSpecialtyVerified ?? []

  function renderCTA() {
    if (checkingAuth) {
      return <div className="py-3 text-center text-slate-500 text-xs animate-pulse">Verificando...</div>
    }
    if (hasApplied) {
      return (
        <div className="space-y-2">
          ...
          <button
            onClick={async () => {
              ...
              if (onWithdraw) { onWithdraw(shift.id); onClose() }
            }}
    ...
    if (!isGeneralShift && specArr.length > 0) {
      const hasDeclared = specArr.some(s => specialtiesMatch(s, shiftSpecialty))
      if (!hasDeclared) {
        return (
      ...
      const isSpecVerified = specVerified.some(s => specialtiesMatch(s, shiftSpecialty))
      if (!isSpecVerified) {
        return (
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 text-center">
            ...
            <p className="text-amber-700/80 text-[11px] mt-0.5">
              Tu especialidad <strong>{shiftSpecialty}</strong> aún no fue validada por el equipo de Guardian.
            </p>
          </div>
        )
      }
    ...
    return (
      <button
        ...
        className="w-full bg-slate-900 hover:bg-blue-700 text-white py-3 rounded-xl font-black text-sm transition-all shadow-lg hover:-translate-y-0.5 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:translate-y-0"
      >
    ...
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md flex items-center justify-center p-3 z-50 overflow-y-auto">
      <div className="bg-white border border-slate-200 rounded-2xl w-full max-w-5xl shadow-2xl animate-in fade-in zoom-in-95 relative my-4 overflow-hidden flex flex-col md:flex-row">
        ...
        <div className="relative h-52 md:min-h-[280px] md:w-64 lg:w-80 shrink-0 overflow-hidden">
          {coverUrl ? (
            <Image src={coverUrl} alt={shift?.clinic?.full_name || 'Clínica'} fill className="object-cover" sizes="(max-width: 768px) 100vw, 320px" unoptimized />
          ) : (
        ...
        <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
          <div className="flex-1 p-5 space-y-4 border-b md:border-b-0 md:border-r border-slate-100 overflow-y-auto">
            ...
            <div className="grid grid-cols-2 gap-2">
              <div className="flex items-center gap-2 bg-slate-50 p-2.5 rounded-xl border border-slate-100">
                ...
                <div>
                  <p className="text-xs font-bold text-slate-800">
                    {shift?.starts_at ? format(parseISO(shift.starts_at), 'dd/MM/yyyy') : '—'}
                  </p>
                  {shift?.starts_at && (
                    <p className="text-[11px] text-slate-500">
                      {format(parseISO(shift.starts_at), 'HH:mm')} — {shift?.ends_at ? format(parseISO(shift.ends_at), 'HH:mm') : '—'} hs
                    </p>
                  )}
                </div>
              </div>
              <div className="flex items-center gap-2 bg-slate-50 p-2.5 rounded-xl border border-slate-100">
                ...
                <div>
                  <p className="text-xs font-bold text-slate-800">{durationHours} hs</p>
          ...
          <div className="w-full md:w-60 lg:w-68 p-5 flex flex-col gap-4">
            <div className="rounded-xl border border-slate-100 bg-slate-50 p-3 space-y-2.5">
              ...
              {clinicProfile.resources.length > 0 && (
